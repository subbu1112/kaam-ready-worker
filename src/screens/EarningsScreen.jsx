...
export default function EarningsScreen({ user, profile }) {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: bData }, { data: pData }] = await Promise.all([
      sb.from('bookings').select('id,service,amount,status,created_at,customer_name,city').eq('worker_id', user.id).gte('created_at', from.toISOString()).order('created_at', { ascending: false }),
      sb.from('payouts').select('id,amount,status,paid_at,created_at,utr,week_start,commission_amount').eq('worker_id', user.id).gte('created_at', from.toISOString()).order('created_at', { ascending: false }),
    ])
    setBookings(bData || [])
    setPayouts(pData || [])
    setLoading(false)
  ...
  function downloadCSV() {
    const rows = [['Date','Service','Customer','Gross','Earnings(90%)'], ...completed.map(b => [new Date(b.created_at).toLocaleDateString('en-IN'), b.service, b.customer_name||'', b.amount||0, Math.round((b.amount||0)*0.9)])]
    const csv = rows.map(r => r.map(c => '"'+c+'"').join(',')).join('\n')
    const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv],{type:'text/csv'})); a.download = 'earnings.csv'; a.click()
  }
      ...
      <div style={{ background:BK, padding:'28px 20px 16px', flexShrink:0 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div>
            ...
            <p style={{ fontSize:12, color:'#555', marginTop:2 }}>Wallet: <span style={{ color:GREEN, fontWeight:700 }}>{fmt(profile?.wallet_balance)}</span></p>
          </div>
          <button onClick={downloadCSV} style={{ background:Y, border:'none', borderRadius:10, padding:'8px 14px', fontWeight:700, fontSize:12, cursor:'pointer', fontFamily:'inherit' }}>⬇ CSV</button>
        </div>
        <div style={{ display:'flex', gap:6, marginTop:14, overflowX:'auto' }}>
          {PERIODS.map(p => (
            <button key={p} onClick={() => setPeriod(p)} style={{ padding:'6px 14px', borderRadius:20, border:'1.5px solid '+(period===p?Y:'#2a2a2a'), background:period===p?Y:'#1a1a1a', fontSize:11, fontWeight:700, cursor:'pointer', fontFamily:'inherit', color:period===p?BK:'#888', whiteSpace:'nowrap', flexShrink:0 }}>{p}</button>
          ))}
        </div>
      </div>
          ...
          <>
            ...
            <div style={{ display:'flex', gap:8 }}>
              {[['earnings','📋 Jobs'],['payouts','💸 Payouts']].map(([v,l]) => (
                <button key={v} onClick={() => setTab(v)} style={{ flex:1, padding:'10px', borderRadius:12, border:'1.5px solid '+(tab===v?Y:'#2a2a2a'), background:tab===v?Y:'#1a1a1a', fontWeight:700, fontSize:13, cursor:'pointer', fontFamily:'inherit', color:tab===v?BK:'#888' }}>{l}</button>
              ))}
            </div>
            {tab === 'earnings' && (
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                {completed.length === 0 && <div style={{ background:'#1a1a1a', borderRadius:14, padding:28, textAlign:'center' }}><p style={{ fontSize:32 }}>📭</p><p style={{ color:'#555', fontSize:13, marginTop:8 }}>No completed jobs in this period</p></div>}
                {completed.map(b => (
            ...
            {tab === 'payouts' && (
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                ...
                {payouts.length === 0 && <div style={{ background:'#1a1a1a', borderRadius:14, padding:28, textAlign:'center' }}><p style={{ fontSize:32 }}>💳</p><p style={{ color:'#555', fontSize:13, marginTop:8 }}>No payouts in this period</p></div>}
                {payouts.map(p => (
                  <div key={p.id} style={{ background:'#1a1a1a', borderRadius:14, padding:'14px 16px', border:'1px solid #2a2a2a' }}>
                    <div style={{ display:'flex', justifyContent:'space-between' }}>
                      <div>
                        <p style={{ fontWeight:700, color:'#fff', fontSize:14 }}>{p.status==='paid'?'✅ Paid':'⏳ Pending'}</p>
