      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#111' }}>
        <div style={{ background: BK, padding: '48px 20px 20px', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              <h1 style={{ fontSize: 18, fontWeight: 800, color: Y }}>{selected.service}</h1>
              <p style={{ fontSize: 12, color: '#636366', marginTop: 4 }}>
        ...
        <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ background: '#1a1a1a', borderRadius: 16, padding: 16, border: '1px solid #2a2a2a' }}>
            {[
              ...
              <div key={k} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '10px 0', borderBottom: '1px solid #2a2a2a' }}>
                <span style={{ fontSize: 12, color: '#636366', flexShrink: 0, marginRight: 12 }}>{k}</span>
                <span style={{ fontSize: 13, fontWeight: 600, color: '#fff', textAlign: 'right', maxWidth: '65%' }}>{v}</span>
              </div>
            ))}
            {selected.amount > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0' }}>
                <span style={{ fontSize: 12, color: '#636366' }}>Amount Earned</span>
                <span style={{ fontSize: 22, fontWeight: 800, color: Y }}>₹{Math.round((selected.amount || 0) * 0.9)}</span>
              </div>
            )}
            {selected.price_note && (
              <div style={{ background: '#111', borderRadius: 10, padding: '10px 12px', marginTop: 4 }}>
                <p style={{ fontSize: 11, color: '#636366', marginBottom: 4 }}>JOB NOTE</p>
                <p style={{ fontSize: 13, color: '#fff' }}>{selected.price_note}</p>
              </div>
            )}
          </div>
          ...
          {(selected.photo_before_url || selected.photo_after_url) && (
            <div style={{ background: '#1a1a1a', borderRadius: 16, padding: 16, border: '1px solid #2a2a2a' }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: '#636366', marginBottom: 10 }}>JOB PHOTOS</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[['Before', selected.photo_before_url], ['After', selected.photo_after_url]].map(([lb, url]) => url && (
                  <div key={lb}>
                    <p style={{ fontSize: 10, fontWeight: 700, color: '#636366', marginBottom: 4 }}>{lb.toUpperCase()}</p>
          ...
          {selected.rating && (
            <div style={{ background: '#1a1a1a', borderRadius: 16, padding: 16, border: '1px solid #2a2a2a', textAlign: 'center' }}>
              <p style={{ fontSize: 12, color: '#636366', marginBottom: 6 }}>CUSTOMER RATING</p>
              <p style={{ fontSize: 28 }}>{'⭐'.repeat(selected.rating)}{'☆'.repeat(5 - selected.rating)}</p>
              <p style={{ color: Y, fontSize: 16, fontWeight: 800, marginTop: 4 }}>{selected.rating} / 5</p>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#111' }}>
      ...
      <div style={{ background: BK, padding: '48px 20px 16px', flexShrink: 0 }}>
        ...
        <h1 style={{ fontSize: 22, fontWeight: 800, color: Y }}>📋 Job History</h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginTop: 14 }}>
          {[
            ...
            <div key={l} style={{ background: '#1a1a1a', borderRadius: 10, padding: '10px 8px', textAlign: 'center' }}>
              <div style={{ fontSize: 16, fontWeight: 800, color: Y }}>{v}</div>
      ...
      {/* Filter tabs */}
      <div style={{ display: 'flex', gap: 8, padding: '10px 16px', background: '#111', flexShrink: 0 }}>
        {[['all', 'All'], ['completed', '✓ Done'], ['active', '🔵 Active'], ['cancelled', '✕ Cancelled']].map(([v, l]) => (
          <button key={v} onClick={() => setFilter(v)}
            style={{ padding: '6px 12px', borderRadius: 20, border: '1.5px solid ' + (filter === v ? Y : '#2a2a2a'),
              background: filter === v ? Y : '#111', fontSize: 11, fontWeight: 700, cursor: 'pointer',
              fontFamily: 'inherit', color: filter === v ? BK : '#888', flexShrink: 0 }}>
            {l}
          </button>
        ))}
      </div>
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '8px 16px 16px' }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: 48 }}>
            ...
            <p style={{ fontWeight: 700, color: '#fff', fontSize: 16 }}>No jobs found</p>
              ...
              return (
                <div key={j.id} onClick={() => setSelected(j)}
                  style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px',
                    borderBottom: i < filtered.length - 1 ? '1px solid #2a2a2a' : 'none', cursor: 'pointer' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontWeight: 700, fontSize: 14, color: '#fff', marginBottom: 2 }}>{j.service}</p>
