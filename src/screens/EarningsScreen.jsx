...
export default function EarningsScreen({ user, profile }) {
  ...
  const [tab,      setTab]      = useState('earnings') // earnings | payouts
  ...
  const pending    = bookings.filter(b => ['searching','assigned','arrived','in_progress'].includes(b.status))

  const totalGross = completed.reduce((s, b) => s + (b.amount || 0), 0)
  const myEarnings = Math.round(totalGross * 0.90)
  const commission = Math.round(totalGross * 0.10)
